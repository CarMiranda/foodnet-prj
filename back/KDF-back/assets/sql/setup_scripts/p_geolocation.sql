...
BEGIN
    ...
    SELECT `id`, 12733.1 * ASIN(SQRT(POWER(SIN(RADIANS(ulat - `users`.`lat`) / 2), 2) + COS(RADIANS(ulat)) * COS(RADIANS(`users`.`lat`)) * POWER(SIN(RADIANS(ulon - `users`.`lon`) / 2), 2))) AS `distance`
    FROM `users`
